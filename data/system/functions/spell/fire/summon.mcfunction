...
data modify entity @e[tag=MotionSimulator,tag=Immature,limit=1] PortalCooldown set value 80
execute as @e[tag=MotionSimulator,tag=Immature,limit=1] run function system:base/motion/save_motion

tag @e[tag=MotionSimulator,tag=Immature,limit=1] add Fireball

# Entity Tracking
summon minecraft:armor_stand ~ ~ ~ {Passengers: [{id: "minecraft:item", Item: {id: "minecraft:magma_cream", Count: 1}, PickupDelay: 32767s, HasVisualFire: true, Invulnerable: true, Silent: true, NoGravity: true, Tags: [FireballEffect]}], PortalCooldown: 80, NoGravity: true, Invulnerable: true, Silent: true, Invisible: true, Marker: true, Tags: [FireballTracker, Tracker, Immature]}
